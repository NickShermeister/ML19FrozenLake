...
class Map(object):
	...
	def populate_rooms(self, game, wall_ratio=0.0):
		self.populate_wall(game)
					...
					for y1 in range(y, y+h):
						if y1 >= 1 and y1 < ymax:
							if not self.get((x1, y1)):
								Tile(game, x1, y1)
		self.populate_random(game, 0.9, wall_ratio=wall_ratio)

	def populate_path(self, game, difficulty=1, wall_ratio=0.2, walls=False):
		self.populate_wall(game)
		xmax = len(self.cells)-1
		ymax = len(self.cells[0])-1
		SEED_DENSITY = 0.07
